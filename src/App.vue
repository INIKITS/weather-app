...
<button type="button" id="search-button" @click="fetchLocation" class=" opacity-70 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full">butt</button>
</div>
...
export default {
  ...
  methods: {
      ...
      async fetchLocation () {
      try {
        // const response = await fetch(this.baseUrl + '/current.json/key=' + this.password + '/q='+this.query)
        // console.log('response :>> ', response);
      const response = await fetch(this.coordinatesUrl + this.query + '&limit=1' + '&appid=' + this.password)
    ...
    this.fetchWeather(this.coordinatesLat, this.coordinatesLong)
      ...
      async fetchWeather ( lat, long) {
        try {
          ...
          const response = await fetch(this.baseUrl + 'lat=' + lat + '&lon=' + long +'&exclude=minutely,hourly,daily,alerts' + '&appid=' + this.password)
          const data = await response.json();
